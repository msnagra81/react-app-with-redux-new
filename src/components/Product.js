import React, { Component } from 'react';
import Title from './functionalComponents/Title'
import styled from 'styled-components';
import {Link} from 'react-router-dom'
import PropTypes from 'prop-types'
import {ProductConsumer} from '../context/context'
import { connect } from 'react-redux';
import {addItemToCart} from '../actions/cartActions'
import {setDetails} from '../actions/setDetailActions'

class Product extends Component {
    constructor(props){
        super(props);
    }
    render() {
        const {id, title, img, price, inCart}= this.props.product;
        return (
            <ProductWrapper  className="col-9 mx-auto col-md-6 col-lg-3 my-3">
                <div>
                        <div className="card">
                        <div className="img-container p-5" onClick={()=>this.props.setDetails(this.props.product)}>   
                            <Link to='/details'>
                                <img src={img} alt='Product Image' className="card-img-top"/>
                            </Link> 
                            <button className="cart-btn" disabled={inCart? true: false} 
                            onClick={()=> {
                                this.props.addItemToCart(this.props.cartItems,this.props.product);
                            //     value.openModal(id);
                            }}
                            >
                            {inCart ? <p className="text-capitalize mb-0" disabled>{" "}in cart</p> : <i className="fas fa-cart-plus"></i>}
                            </button>
                        </div>
                        {/* Card Footer */}
                        <div className="card-footer d-flex justify-content-between">
                            <p className="align-self-center mb-0">{title}</p>
                            <h5 className="text-blue font-italic mb-0">
                                <span className="mr-1">$</span>
                                {price}</h5>
                        </div>
                        </div> 
                </div>              
            </ProductWrapper>
        );
    }
}

const mapStateToProps = state => ({
    currentState: state,
    cartItems: state.fetchData.cart
})
export default connect(mapStateToProps,{setDetails,addItemToCart})(Product);

Product.propTypes = {
    product: PropTypes.shape({
        id: PropTypes.number,
        img: PropTypes.string,
        title: PropTypes.string,
        price: PropTypes.number,
        inCart: PropTypes.bool,
    }).isRequired
}
const ProductWrapper = styled.div`
.card {
    border-color: transparent;
    transition: all 1s linear;
}
.card-footer{
    background: transparent;
    border-color: transparent;
}
&:hover{
    .card{
        border: 0.04rem solid rgba(0,0,0,0.2);
        box-shadow: 2px 2px 5px 0px rgba(0,0,0,0.2)
    }
    .card-footer{
        background: rgba(247,247,247);
        border: 0.02rem solid rgba(0,0,0,0.2);
        box-shadow: 2px 2px 5px 0px rgba(0,0,0,0.2)
    }
}
.img-container{
    position: relative;
    overflow: hidden;
}
.card-img-top{
    transition: all 0.5s linear;
}
.img-container: hover .card-img-top{
    transform: scale(1.2);
}
.cart-btn{
    position: absolute;
    bottom: 0;
    right: 0;
    padding: 0.2rem 0.4rem;
    background: var(--lightBlue);
    border: none;
    color: var(--mainWhite);
    font-size: 1.4rem;
    border-radius: 0.5rem 0 0 0;
    // display: none;
    tarnsform: translate(100%, 100%);
    transition: all 1s linear;
}
.img-container: hover .cart-btn{
     transform: translate(0,0);
    // display: inline;
}
`;